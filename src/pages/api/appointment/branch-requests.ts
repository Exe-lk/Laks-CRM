...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Fetch appointments for the specific branch
    ...
    const formattedRequests = requests.map(request => {
      ...
      return {
        ...
        practice: request.practice ? {
          id: request.practice.id,
          name: request.practice.name,
          email: request.practice.email,
          telephone: request.practice.telephone
        } : null,
    ...
    res.status(500).json({ error: "Failed to fetch branch requests" });
  }
}
